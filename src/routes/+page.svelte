<script>
	...
	import { recommendedCoins, preferredCoinData } from "$lib/store.ts";
		...
		<div class="xl:grid xl:grid-cols-2 xl:grid-rows-2 xl:h-screen xl:mb-8">
			<div class="xl:col-start-1 xl:grid xl:grid-cols-2 overflow-hidden bg-gradient-to-r from-white to-gray-50 shadow-xl rounded-xl mx-2 xl:mx-4 mt-4 border-4 border-gray-100">
				<div class="flex flex-col justify-center items-center px-4 py-6 xl:grid-start-1">
					...
					<h4 class="font-semibold text-xl italic">{$preferredCoinData.ask?.toFixed(3) || data.ask.toFixed(3)}</h4>
				</div>
					...
					<div class="px-2 py-2 space-y-2 w-full">
						<div class="relative flex items-center justify-center space-x-3 rounded-lg border border-gray-300 bg-white shadow-sm hover:border-gray-400">
							<div class="min-w-0 flex flex-col text-center p-4">
								<p>Bid</p>
								<p class="italic">{$preferredCoinData.bid?.toFixed(3) || data.bid.toFixed(3)}</p>
							</div>
						</div>
						<div class="relative flex items-center justify-center space-x-3 rounded-lg border border-gray-300 bg-white shadow-sm hover:border-gray-400">
							<div class="min-w-0 flex flex-col text-center p-4">
								<p>Rate</p>
								<p class="italic text-sm truncate">{$preferredCoinData.rate || data.rate}</p>
							</div>
						</div>
						<div class="relative flex items-center justify-center space-x-3 rounded-lg border border-gray-300 bg-white shadow-sm hover:border-gray-400">
							<div class="min-w-0 flex flex-col text-center p-4">
								<p>Spot Rate</p>
								<p class="italic">{$preferredCoinData.spotRate?.toFixed(3) || data.spotRate.toFixed(3)}</p>
							</div>
						</div>
						<div class="relative flex items-center justify-center space-x-3 rounded-lg border border-gray-300 bg-white shadow-sm hover:border-gray-400">
							<div class="min-w-0 flex flex-col text-center p-4">
								<p>Date</p>
								<p class="italic">{new Date($preferredCoinData.timestamp || data.timestamp).toDateString()}</p>
							</div>
						</div>
					</div>
				</div>
